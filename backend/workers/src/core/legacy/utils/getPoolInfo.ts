...
export const getPoolKeys = async (connection: Connection, baseMint: PublicKey) => {
  try {
    const res = await fetch(
      `https://api.dexscreener.com/latest/dex/tokens/${baseMint.toBase58()}`,
      {
        method: 'GET',
        headers: {
    ...
    const data = (await res.clone().json()) as DexScreenerResponse;
    if (data.pairs.length == 0) {
      ...
      if (!raydiumPair) {
        console.log('No Raydium pair found for token');
        return null;
      }
